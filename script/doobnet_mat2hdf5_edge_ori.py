...
def PIOD_augmentation(mat_filename, h5_dir, img_src_dir, img_dst_dir, val_list):
    ...
    orient_map = mat['bndinfo_pascal']['OrientMap'][0][0].astype(np.float32) ## OrientMap data, double of [H,W]
    height, width = orient_map.shape
    imsize = mat['bndinfo_pascal']['imsize'][0][0][0] ## H,W
    ...
    indices = mat['bndinfo_pascal']['edges'][0][0][0]['indices'][0][0] ## uint32 of [num_edgepixel,1]
    edge_map = np.zeros(imsize, dtype=np.uint8) ## [H,W]
    for idx in range(len(indices)):
        ...
        edge_map[pixel_coords] = 1 ## EdgeMap data
    ...
    label = np.zeros((1, 2,height,width), dtype=np.float32) ## [1,2,H,W]
    label[0, 0, ...] = edge_map
    label[0, 1, ...] = orient_map
    ## now get the origin label data
    ...
    ## if image size < 320, then should resize
    ## the interpolation method can notice
    ...
    ### so the h5 file only has ['label'] for Orientation and Edge map, and the name of mat_id
...
def gt_flip(edge_ori):
    ...
    mask = edge == 1
    ori[mask] = -ori[mask] ## notice: the orientation of flip image's edge is inversed
...
def cal_height_width(height, width, shortest_side):
    ## should keep the aspect  -> calculate the new H W
...
def gt_rescale(edge_ori, shortest_side):
    ...
    ## maybe the edge map should be 0 or 1
...
def main():
    ...
    root_path = '/home/yuzhe/Downloads/PIOD/PIOD'
    output_dir = os.path.join(root_path,'Augmentation')
    label_dir = os.path.join(root_path,'Data')
    img_dir = os.path.join(root_path,'Images')
    piod_val_list_file = os.path.join(root_path,'val_doc_2010.txt') ## txt list of data filename (.jpg and .mat)
    ...
    if (args.dataset == 'BSDSownership'):
        ...
        mat_list = glob.glob(os.path.join(label_dir, '*.mat'))
        for mat_path in mat_list:
            BSDS__augmentation(mat_path, dst_label_dir, img_dir, dst_img_dir)
        ...
        for mat_path in mat_list:
            ...
            img_path = os.path.join(os.path.abspath(img_dir), '{}.jpg'.format(iid))
            test_list.append(img_path)
            test_iids.append(iid)

        # save to file
        with open(os.path.join(output_dir, 'train_pair.lst'), 'w') as f:
            for img_path, gt_path in train_val_pair_list:
                f.write('{} {}\n'.format(img_path, gt_path))
        print('Write train list to {}.'.format(os.path.join(output_dir, 'train_pair.lst')))
        ...
        with open(os.path.join(output_dir, 'test_ori_iids.lst'), 'w') as f:
            for iid in test_iids:
                f.write('{}\n'.format(iid))
        print('Write test ids to {}.'.format(os.path.join(output_dir, 'test_ori_iids.lst')))


    elif (args.dataset  == 'PIOD'):
        val_list = []
        if (not piod_val_list_file):
            print('Require val_doc_2010.txt. For usage, see --help!')
            sys.exit(-1)

        assert(os.path.exists(piod_val_list_file))
        ...
        for mat_path in mat_list: ## all .mat file
            print mat_path
            PIOD_augmentation(mat_path, dst_label_dir, img_dir, dst_img_dir, val_list)
            ## process all data

        print('train: {} val: {}'.format(train_cnt, val_cnt)) ## train: 9169 val: 925
        print('boundary pixel rate: {}'.format(float(total_edge_pixel)/total_pixel))  ## domn't ignore this, for alpha in the loss !!!
        ...
        for h5_path in h5_list:
            ...
            if (h5_id in val_list):
                ...
                gt_path = os.path.join(os.path.abspath(dst_label_dir), h5_filename)
                train_val_pair_list.append((img_path, gt_path))
        with open(os.path.join(output_dir, 'train_pari_320x320.lst'), 'w') as f:
            for img_path, gt_path in train_val_pair_list:
                f.write('{} {}\n'.format(img_path,  gt_path))
        print('Write train list to {}.'.format(os.path.join(output_dir, 'train_pari_320x320.lst')))
...
def write_lst():
    ...
    train_val_list = sorted(train_val_list)
    with open(os.path.join(output_dir, 'train_pari_320x320.lst'), 'w') as f:
        for img_path in train_val_list:
            f.write('{}\n'.format(img_path))
    print('Write train list to {}.'.format(os.path.join(output_dir, 'train_pari_320x320.lst')))
